...
var MVPmat = function ( dispParams ) {
	...
	function computeViewTransform( state, halfIpdShift ) {
		...
		//lookAt( vec3(-halfIpdShift,0,d), vec3(-halfIpdShift,0,0),upVector );

		var upVector = new THREE.Vector3( 0, 1, 0 );

		var viewerPosition = new THREE.Vector3(state.viewerPosition.x-halfIpdShift, state.viewerPosition.y, state.viewerPosition.z);

		var viewerTarget = new THREE.Vector3(state.viewerTarget.x-halfIpdShift, state.viewerTarget.y, state.viewerTarget.z);
	...
	this.update = function ( state, renderingMode ) {
		...
		if ( renderingMode === STANDARD_MODE
			...
			var rightReye = - leftLeye;

			var leftReye = - rightLeye;
			...
			this.anaglyphProjectionMat.L = computePerspectiveTransform(
				leftLeye, rightLeye, top, bottom, state.clipNear, state.clipFar );

			this.anaglyphProjectionMat.R = computePerspectiveTransform(
				leftReye, rightReye, top, bottom, state.clipNear, state.clipFar );
