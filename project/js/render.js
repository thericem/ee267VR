...
var distanceScreenViewer = 500.0;
...
var dofChromaRenderer =
	new DofChromaRenderer( webglRenderer, dispParams );
...
function animate() {
	...
	if ( renderingMode === STANDARD_MODE ) {
		...
		// First rendering pass (into FBO)
		standardRenderer.renderOnTarget(
			dofChromaRenderer.renderTarget,	sc.state,
			mat.modelMat, mat.viewMat, mat.projectionMat );

		// Second rendering pass (onto screen)
		dofChromaRenderer.render( sc.state, mat.projectionMat );
		...
		// Second rendering pass (onto screen)
		foveatedRenderer.render( sc.state, mat.projectionMat );
